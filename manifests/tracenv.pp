...
#
# Modified for external db host and schema
# 2015-07-06, Tim Holloway <timh@mousetech.com>
#
# === Parameters
#
# [*adjust_selinux*]
...
#
# [*db_host*]
#   The host for the database if itś not a local file. Defaults to ´localhost´.
#
# [*db_schema*]
#   The schema used by this Trac instance. Defaults to primary schema for the
#   database. Used as option in Postgresql.
#
# [*db_loc*]
...
define trac::tracenv(
  ...
  if ($db_type == 'sqlite') {
    ...
    $dburi = "$db_type://$db_user:$db_password@$db_host/$_db_file$_db_schema"
  }
  ...
  # Create db (timh - moved because trac-admin needed psycopg driver!)
  if $create_db {
    trac::db{$name:
